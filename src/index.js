...
class ServerlessS3Local {
  ...
  subscriptionWebpackHandler() {
    return new Promise((resolve) => {
      ...
      this.unsubscribe()
      this.subscribe()
      this.serverless.cli.log('S3 event listeners refreshed')
      resolve()
    })
  }
  ...
  subscribe() {
    ...
    this.s3eventListener = (event) => {
      ...
      this.eventHandlers
        ...
        .filter((handler) => ServerlessS3Local.rulesMatch(handler.rules, key))
        .forEach((handler) => {
          handler.func(event)
        })
    }

    this.client.on('event', this.s3eventListener)
  }

  unsubscribe() {
    if (this.client && this.s3eventListener) {
      this.client.removeListener('event', this.s3eventListener)
      this.s3eventListener = null
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      const configs = [
        cors
          ...
          : null,
        website
          ? fs.readFileSync(path.resolve(this.serverless.config.servicePath, website), 'utf8')
          : null,
      ].filter((x) => !!x)
  ...
  endHandler() {
    if (!this.options.noStart && this.client) {
      this.unsubscribe()
      this.client.close()
      this.client = null
        ...
        buckets.map(async (bucket) => {
          ...
          try {
            ...
            do {
              ...
              const objects = (listed.Contents || []).map(({ Key }) => ({
                Key,
              }))
              if (objects.length) {
                await s3Client.send(
    ...
    this.service.getAllFunctions().forEach((functionKey) => {
      ...
      const func = async (s3Event) => {
        ...
        try {
          ...
          try {
            ...
            Object.keys(originalEnvironment).forEach((key) => {
      ...
      ;(functionDefinition.events || []).forEach((event) => {
        ...
        s3Events.forEach((existingEvent) => {
          ...
          eventHandlers.push(ServerlessS3Local.buildEventHandler(name, pattern, s3Rules, func))
        })
  ...
  static buildEventHandler(name, pattern, s3Rules, func) {
    const rule2regex = (rule) =>
      Object.keys(rule).map(
        (key) =>
          (key === 'prefix' && { prefix: `^${ServerlessS3Local.escapeRegex(rule[key])}` }) || {
            suffix: `${ServerlessS3Local.escapeRegex(rule[key])}$`,
          }
      )
  ...
  static rulesMatch(rules, key) {
    const matches = rules.reduce(
      (acc, rule) => {
      ...
      {
        prefix: !rules.some((rule) => rule.prefix),
        suffix: !rules.some((rule) => rule.suffix),
      }
    )
    return Boolean(matches.prefix && matches.suffix)
  }
  ...
  getResourceForBucket(bucketName) {
    ...
    return this.service.resources?.Resources
  ...
  hasPlugin(pluginName, strict = false) {
    ...
    return this.service?.plugins?.modules
  ...
  buckets() {
    ...
    const eventSourceBuckets = Object.keys(this.service.functions || {}).reduce((acc, key) => {
      const serviceFunction = this.service.getFunction(key)
      const buckets = (serviceFunction.events || [])
        .map((event) => {
          const s3 = (event && (event.s3 || event.existingS3)) || undefined
          if (!s3) {
            return null
  ...
  setOptions() {
    const config = this.serverless.service.custom?.s3 || {}
    this.options = {
      ...defaultOptions,
